...
def front():
    ...
    detailsStatus, details = getProductDetails(product_id if not is_masquerading else 372, headers)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("usage: {} port num_pod_replications".format(sys.argv[0]))
        sys.exit(-1)

    port, num_pod_replications = int(sys.argv[1]), int(sys.argv[2])
    sys.stderr = Writer('stderr.log')
    sys.stdout = Writer('stdout.log')
    is_masquerading = random.random() <= 1.0 / num_pod_replications
